...
| 视图 | 功能 |
|---|---|
| **账号池** | 统计条（总数/可用/冷却/禁用/可用积分合计/粘性会话）+ **积分使用量看板**（近 5 小时 / 近 24 小时消耗 + 账单累计已用）+ 账号表：状态标签（可用/限流冷却/积分冷却/熔断/已禁用）、积分量条、成功失败计数、在途、单号操作（签到/余额/任务/解冻/禁用/移除）；批量「全部签到」「旅行巡检」「活跃上报」「全部保活」 |
...
| **成员管理** | 为每位使用者签发独立密钥（`wb-` 前缀），并按成员监控用量（近 5 小时 / 近 24 小时 / 累计）；支持改名、重置密钥、清零用量、删除。见[成员管理](#成员管理) |
| **配置** | 在线编辑 config.json：API 密钥、定时任务（四类任务时点与开关、余额刷新间隔）、账号池与流量治理参数、上游超时与 UA、提示词模式、脱敏/粘性开关 |
| **运行日志** | 最近 500 行服务日志 + 请求表格日志（可开关自动滚动） |

### 积分使用量看板

账号池页顶部展示三个指标：**近 5 小时消耗**、**近 24 小时消耗**、**当前累计已用**。

数据来源是**上游计费口径的 `used` 计数**（`billing/meter/get-user-resource`）定期采样，而非累加每次请求的 `usage.credit`——这样能涵盖**全部消耗来源**，包括网关定时任务（夜猫子等）与任何非 `/v1/chat/completions` 路径的花费。采样每 **5 分钟**一次，序列落盘在 `data/usage.json`（与 `state.json` 同目录，重启保留）。

窗口值 = 各账号「当前 used − 窗口起点 used」之和。逐账号比对而非直接比总量，使窗口内增删账号不会制造虚假消耗；套餐周期重置导致的负增量钳为 0（宁可少算，不虚报）。未满窗口时标注 `complete: false` 并显示实际覆盖时长。

**已知粒度限制**：上游计费有分钟级延迟，`used` 为整数分，故窗口值是近似值（±数分），**不适用于逐请求精确计费**；刚启动时历史不足，需累积数小时才满窗口。

| 接口 | 鉴权 | 说明 |
|---|---|---|
| `GET /panel/api/usage` | Bearer（同一 `api_key`） | 使用量看板数据（仅读内存样本，不打上游） |

### 成员管理

为每位使用者签发独立密钥，并按其维度监控用量——解决"单一 `api_key` 无法区分谁在用"的问题。

**权限边界**（重要）：成员密钥**只能**访问 `/v1/chat/completions` 与 `/v1/models`；
`/status` 与全部 `/panel/api/*` 一律要求管理员 `api_key`，成员访问返回 403 / 401。
这样成员看不到账号池、冷却状态、配置与其他成员信息。管理员密钥不受此白名单限制。

成员接入方式与管理员一致，只是换成自己的密钥：

```
Base URL：http://<主机>:7863/v1
API Key ：wb-xxxxxxxx（成员密钥）
```

**记账口径**：按每次响应的 `usage.credit` 归因到请求发起者。为什么不用上游账单 `used`——
`used` 只能按账号（上游凭证）聚合，无法区分同一账号下的不同调用者；`usage.credit` 是唯一
能归因到"请求发起者"的量。**精度限制**：该值为两位小数，极小请求可能记 0，适合相对观测
而非精确结算。管理员自身请求不计入任何成员。

成员数据落盘在 `data/members.json`（含明文密钥，故 `.gitignore` 已排除整个 `data/`），
用量按 **15 分钟**分桶，保留 24 小时；进程内按 **10 秒**周期落盘。

| 接口 | 鉴权 | 说明 |
|---|---|---|
| `GET /panel/api/members` | Bearer（管理员） | 成员列表 + 汇总（含窗口用量） |
| `POST /panel/api/members` | Bearer（管理员） | 新建成员，返回新密钥（仅此一次明文） |
| `POST /panel/api/members/{id}/update` | Bearer（管理员） | 改名称 / 备注 |
| `POST /panel/api/members/{id}/rotate` | Bearer（管理员） | 重置密钥（旧密钥立即失效） |
| `POST /panel/api/members/{id}/reset` | Bearer（管理员） | 清零用量（保留成员与密钥） |
| `POST /panel/api/members/{id}/remove` | Bearer（管理员） | 删除成员 |

> 列表接口的密钥为掩码（`wb-xxxx••••••xxxx`）；**明文只在创建与重置时回显一次**，
> 请及时分发给对应成员。面板「复制密钥」按钮复制的是列表中的原值，
> 若已不可得，用「重置密钥」重新签发。
...
- 备份 = `auths/`（凭证）+ `data/state.json`（池状态：积分 / 冷却 / 计数）+ `data/usage.json`（使用量采样，可删；删除后看板从零重新累积）+ `data/members.json`（成员与其密钥、用量，**含明文密钥**）；配置 Upstash 后状态另镜像至 Redis（7 天 TTL）
...
- **成员密钥**：`data/members.json` 以**明文**保存各成员密钥（`0600`），与 `config.json` 里的 `api_key` 同等级敏感——该文件已在被排除的 `data/` 目录内，切勿手工拷出仓库外传播
