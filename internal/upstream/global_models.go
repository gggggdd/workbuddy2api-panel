...
func (c *Client) probeGlobalModels(a *auth.Auth) (names []string, infos []ModelInfo, efforts map[string][]string, defaults map[string]string, err error) {
	...
	// （IDE 与 CLI 各有独有模型，见 codeBuddyCLIUA 注释），故 global 侧并发两路取并集。
	...
	// v3 两路自合并：IDE 路字段权威（单条字段更全），CLI 路只补缺失的模型 id。
	...
	if v3.err != nil {
		...
		// 窄表形态（parseGlobalModelNames 只回 names、infos 为 nil）必须回落到
		// enterprise.names，否则 v3 一失败整个目录就空——与"两路皆成功"分支同款回落。
		if len(names) == 0 {
