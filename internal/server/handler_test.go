...
// TestChat429Code14018UsesHardCreditCooldown 端到端回归（上游 issue #175）：
// 上游 429 + code 14018 = 账号积分耗尽，必须走硬积分冷却（至次日 04:00），
// 而不是落进 429 软限流兜底（短冷却后反复撞墙）；无该业务码的普通 429
// 措辞（quota/额度不足）仍保持软限流语义。
