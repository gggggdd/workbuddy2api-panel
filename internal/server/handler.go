...
type Config struct {
	...
	// Members 成员密钥与用量记账（可选；nil = 关闭成员体系）。
	Members *member.Store

	// Ledger 积分账本（可选；nil = 不记账）。
...
// authenticate 三路鉴权：管理员 api_key 全放行；成员密钥走白名单端点；
// api_key 为空且无成员体系 → 放行（未启用鉴权语义）。
...
func (h *Handler) chatCompletions(w http.ResponseWriter, r *http.Request) {
	...
	// 成员用量记账 + 积分账本（fork 特性）：局部变量 + defer 闭包，任何出口都记一次。
	mid := memberOf(r)
	charged := 0.0
	succeeded := false
	...
	for i := 0; i < h.cfg.MaxRotate; i++ {
		...
		if status >= 400 {
			...
			// 同一 body 换账号仍是同样的解析结果，轮转只会把健康账号轮一遍、
			// 最后仍把 503 返回给客户端（issue: 无效图片被打成上游故障）。
		...
		// 响应头透出实际服务的账号：上游选号对客户端原本不可见——管理面板/网关据
		// 此把每次成功请求记到账号名下，排障时直接看到「这次是谁在答」。uid 是
		// 十六进制 ASCII 可入头；昵称可能含非 ASCII，不入头（消费方按 uid 自行
		// 映射）。放在 NoteSuccess 之后：语义是「这个号成功服务了本次请求」——
		// 中途失败换号的账号不冒领。此时未写任何字节，流式（首帧前）与非流式
		//（writeJSON 前）都生效。
...
//     换任何号都会得到相同的解析错误）。零动作（不冷却/不熔断/不 NoteError、不喂
//     连败），chatCompletions 已 fail-fast 透传原文返回不轮转。
...
func (h *Handler) applyErrorPolicy(uid string, kind upstream.ErrKind, body, model string, uerr *upstream.Error) {
	...
	case upstream.ErrImageInvalid:
		...
		// 得到相同解析错误）。零动作（不冷却/不熔断/不 NoteError、不喂连败），
